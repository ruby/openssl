...
class OpenSSL::TestPKey < OpenSSL::PKeyTestCase
  ...
  if openssl?(3, 0, 0)
    def test_s_from_data_with_invalid_alg
      assert_raise_with_message(OpenSSL::PKey::PKeyError, /EVP_PKEY_CTX_new_from_name/) do
        OpenSSL::PKey.from_data("ASR", {})
      end
    end

    def test_s_from_data_argument_errors
      assert_raise(ArgumentError) { OpenSSL::PKey.from_data("RSA") }
      assert_raise(TypeError) { OpenSSL::PKey.from_data("RSA", "not a hash") }
    end

    def test_s_from_data_under_gc_stress
      # The converted values must be protected from GC until
      # EVP_PKEY_fromdata() has consumed them
      new_key = begin
        GC.stress = true
        OpenSSL::PKey.from_data("RSA", "n" => 3161751493, "e" => 65537, "d" => 2064855961)
      ensure
        GC.stress = false
      end
    ...
    def test_s_from_data_ec_pub_given_as_string
      ...
      assert_nil new_key.private_key
    end
