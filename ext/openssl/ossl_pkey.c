...
pkey_gen_cb(EVP_PKEY_CTX *ctx)
{
    ...
    if (arg->yield) {
	VALUE ary;
	int state, i, info_num = EVP_PKEY_CTX_get_keygen_info(ctx, -1);

	ary = rb_ary_new2(info_num);
	for (i = 0; i < info_num; i++)
	    rb_ary_store(ary, i, INT2NUM(EVP_PKEY_CTX_get_keygen_info(ctx, i)));
...
pkey_generate(int argc, VALUE *argv, VALUE self, int genparam)
{
	...
	StringValue(alg);
	ameth = EVP_PKEY_asn1_find_str(&tmpeng, RSTRING_PTR(alg), RSTRING_LENINT(alg));
	if (!ameth)
    ...
    if ((genparam ? EVP_PKEY_paramgen_init(ctx) : EVP_PKEY_keygen_init(ctx)) <= 0) {
	EVP_PKEY_CTX_free(ctx);
	ossl_raise(ePKeyError, genparam ? "EVP_PKEY_paramgen_init" : "EVP_PKEY_keygen_init");
    }
...
ossl_pkey_derive(int argc, VALUE *argv, VALUE self)
{
    ...
    SafeGetPKey(peer_pkey_obj, peer_pkey);
    ...
    /* FIXME: memleak of |ctx| */
    assert(keylen <= LONG_MAX);
    str = rb_str_new(NULL, (long)keylen);
    if (EVP_PKEY_derive(ctx, (unsigned char *)RSTRING_PTR(str), &keylen) <= 0) {
	EVP_PKEY_CTX_free(ctx);
	ossl_raise(ePKeyError, "EVP_PKEY_derive");
    }
    rb_str_set_len(str, (long)keylen);
    EVP_PKEY_CTX_free(ctx);
    return str;
}
...
Init_ossl_pkey(void)
{
    ...
    id_private_q = rb_intern_const("private?");
